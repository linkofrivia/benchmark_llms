...
While Llama dominated in logic, Zephyr proved more truthful, outperforming Llama by over 8% on the TruthfulQA MC2 metric. This highlights the "sycophancy trap" inherent in heavy RLHF pipelines. Because Llama is tuned to be a helpful, polite assistant, it is more likely to "people-please" by agreeing with false premises or common misconceptions. TruthfulQA remains valid despite quantization because it relies on internal knowledge ranking which is more stable than sequential logic. This shows that Zephyr’s simpler DPO alignment is less susceptible to these psychological traps.
